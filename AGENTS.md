...
- `App.tsx` — роуты: `/` = курс (CourseHomePage), `/learn/:m/:t/:i` = урок,
  `/tasks` = плоский каталог заданий, `/quizzes`, `/exercise/:slug`, `/quiz/:slug`.
- `course.ts` — модель курса, ленивые загрузчики (`loadCourse`, `loadTheory`),
  прогресс в localStorage (`readProgress`/`markLessonDone`).
- `pages/CourseHomePage.tsx` — обзор курса (модули→темы→уроки).
- `pages/LessonPage.tsx` — урок: дерево курса слева + теория (react-markdown) +
  степпер ①②③ + кнопки квиз/практика (реюз `/quiz`,`/exercise`) + prev/next + прогресс.
...
**UI-формат курса выбран — линейный (Hexlet/Stepik):** дерево слева + урок со
степпером. Открытый UX-вопрос: встроить квиз/практику прямо в шаги урока (сейчас
переход на отдельные страницы) — обсуждается с заказчиком.
...
- Контент теории — повторяемо из Buildin (скрипт), не править руками в `content/theory`
  (перетрётся при ре-импорте); правки — в Buildin или в генераторах.
