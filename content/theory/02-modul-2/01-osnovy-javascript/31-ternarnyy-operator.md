...
Хочется короче: `return` с выражением, но `if` — инструкция, не выражение.

Аналог `if-else` в форме **выражения** — **тернарный оператор** `? :`.
...
Далее перепишем начальный вариант `getTypeOfSentence()` аналогично:
