...
**Условия** (`if` / `else`) меняют поведение программы в зависимости от предиката.

## if

Пример: тип предложения — вопрос или обычное:
...
`if (предикат) { ... }` — тело выполняется только при true.
...
Рекомендуем **всегда** ставить `{}` у `if`/`else` — так границы условия видны явно.
...
Здесь добавлен ключевое слово `else` и новый блок с фигурными скобками. Этот блок выполнится, только если условие в `if` — ложь.
...
Функция `getTypeOfSentence()` из предыдущего примера различает только вопросительные и обычные предложения. Далее попробуем добавить поддержку восклицательных предложений:
...
Здесь добавлен еще одну проверку ("exclamation" переводится «восклицание»). Технически функция работает, но с точки зрения семантики есть проблемы:
