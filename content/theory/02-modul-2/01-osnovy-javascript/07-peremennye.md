...
Нужно вывести *Father!* несколько раз — можно повторить `console.log`:
...
Повторы усложняют правки. Проще **объявить переменную** с текстом и использовать её:
...
Переменная хранит значение и даёт имя для повторного использования: `let greeting = 'Father!'`.
...
Переменную можно **переприсвоить** — значение меняется, имя остаётся:
...
Переменную нужно объявить **до** использования — иначе ReferenceError:
...
В примерах модуля `let` часто стоит у значений, которые не меняются — по смыслу это **константы**.
...
Зачем такие сложности? Почему бы не оставить только переменные? Даже если бы мы оставили только переменные, то это не отменяет того факта, что они часто использовались бы как константы, более того, код на JavaScript можно и идиоматично писать без использования переменных вообще. Посмотрите на пример из реального кода платформы:
...
С `const` значение не меняется — проще читать код; с `let` нужно отслеживать все присваивания.
