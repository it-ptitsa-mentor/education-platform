...
Код усложняется — пора к **циклам**: повторять действия, пока выполняется условие.

**Алгоритм** — упорядоченные шаги к результату. В программах алгоритмы часто включают повторяющиеся шаги.
