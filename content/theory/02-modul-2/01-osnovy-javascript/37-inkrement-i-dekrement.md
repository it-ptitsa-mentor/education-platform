...
buildin_id: 657b292f-916d-4584-9a48-52c663fa9d4a
source: platform
...
Операции **`++`** и **`--`** из C-подобных языков увеличивают/уменьшают переменную на 1:
...
1. Так как значение изменилось, `console.log(x)` вывел 6.

1. Теперь выводим на экран `x++`. Это постфиксный инкремент, поэтому возвращено значение, содержавшееся в переменной до ее увеличения на 1.

1. Так как значение изменилось, `console.log(x)` вывел 7.
