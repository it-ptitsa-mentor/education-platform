...
Маленький городок: все здания знают по имени — «встретимся у почты».
...
В JavaScript: **один файл — один модуль**. Упражнения курса — отдельные модули.
...
Далее посмотрим на пример, где у нас есть файл `math.js`:
...
Код здесь не будет работать, потому что `surfaceArea` и `square` здесь не существуют. Они в другом файле, о котором JavaScript пока не знает. Нам нужно сказать ему заглянуть в другой файл. Это называется "импортом" — далее импортируем именно те функции, что нам нужны, по их имени:
...
Ниже описан основной алгоритм, по которому нужно анализировать файл с кодом, над которым вы сейчас работаете. Этот алгоритм не привязан к учебной среде платформы — так нужно делать в принципе:
