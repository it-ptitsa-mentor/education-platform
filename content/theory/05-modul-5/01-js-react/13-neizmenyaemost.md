...
Неизменяемость состояния — одна из ключевых тем в React. Её легко придерживаться, работая с примитивными типами данных, но с составными, такими как объекты и массивы, у неподготовленного пользователя могут возникнуть сложности. В этом разделе рассматриваются основные способы частичного обновления объектов и массивов.
...
Заметьте на способ задания обработчика: `removeItem = (id) => (e) => {` и его использование `onClick={this.removeItem(id)}`.
...
[https://www.npmjs.com/package/immutability-helper](https://www.npmjs.com/package/immutability-helper)
