...
В курсе [JS: DOM API](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model) говорилось, что при работе с *HTML* предпочтительно использовать `addEventListener`. Одна из главных причин заключается в том, что такой способ позволяет повесить множество обработчиков, чем и пользуются многие JS-дополнения. В React такой способ работы просто не нужен, так как управление потоком событий всегда явное. Никто не может подключиться к React со стороны и навесить туда своих обработчиков.
