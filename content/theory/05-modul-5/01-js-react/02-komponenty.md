...
Откройте пример, который будет разбираться в течение урока:

Пример можно открыть в CodePen с подключённым React.
Главное понятие в React — компонент. Это единственная сущность, которую он содержит. Вся остальная функциональность построена вокруг компонентов.

В примере компонент выводит `<div>Hello!</div>` в DOM страницы.
...
Для браузера нужны две зависимости: `React` и `ReactDOM`. Ядро React не привязано к DOM и работает не только в браузере; отрисовку в DOM вынесли в пакет ReactDOM.
...
1. Функция `render` возвращает нечто (разберём позже), что будет отрисовано в браузере. Класс-компонент без функции `render` существовать не может, это его интерфейс
