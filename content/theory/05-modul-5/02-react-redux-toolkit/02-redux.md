...
Redux часто сравнивают с клиентской «базой данных»: в **store** лежит **состояние** — все данные приложения в одном месте. Эта база отвечает только за данные, поэтому она никак не связана с браузером, DOM и фронтендом в целом. Теоретически Redux можно использовать даже на бэкенде в Node.js.

В коде Redux — это **store** (хранилище). Компоненты и модули читают данные через `getState()` и меняют их только через `dispatch()`. В терминологии Redux этот объект называется **хранилищем** (*store*).
...
Разберём полный пример использования Redux:
...
Изменить состояние можно только одним способом: вызвать `dispatch()` с **action** (действием).
...
Разберём еще один пример с использованием массива и передачей данных через действие:
...
Перейдём к **начальному состоянию**. Выше мы упоминали, что оно задается в определении редьюсера:
