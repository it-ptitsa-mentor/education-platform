...
Наконец, перейдем к экспортам. Функция `createSlice()` генерирует редьюсер и действия к нему. Все это [официальная документация](https://redux-toolkit.js.org/tutorials/quick-start) рекомендует экспортировать так:
...
- [Как проверить state внутри reduce?](https://ru.hexlet.io/qna/javascript/questions/kak-uvidet-state-vnutri-reduce)
