...
**RTK Query** — слой для HTTP-запросов поверх Redux Toolkit: кэш, статусы загрузки и хуки для компонентов. Он позволяет создавать удобные интерфейсы, благодаря которым мы можем отслеживать состояния запросов и при этом не создавать много однообразного кода.
...
RTK Query имеет множество различных настроек и функций. В этом разделе мы не будем подробно разбирать весь функционал, а коснемся только основного.

## Создание API

Разберём создание простого API для четырех роутов:
