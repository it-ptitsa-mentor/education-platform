---
title: "Дженерики (Функции)"
module: "Модуль 6"
topic: "Продвинутый Typescript"
buildin_id: 0093e7fd-3789-4f60-a7d0-b1411189149f
source: platform
...
Без дженериков один и тот же алгоритм пришлось бы копировать для каждого типа данных — дублирование кода неизбежно.

Функция `last()` возвращает последний элемент массива. Обобщённая версия:
...
То же поведение на обычных функциях потребует отдельной функции на каждый тип — с уникальным именем:
...
**Параметрический полиморфизм** — когда одна функция обрабатывает значения разных типов одним алгоритмом. Дженерики в TypeScript — его реализация.
