...
Для других состояний также существуют свои псевдоклассы.
...
Основным структурным псевдоклассом — это `:nth-child(условие)`. Вы можете увидеть, что это целая функция, которая принимает условие. По этому условию будет выбран элемент или элементы. Разберемся, какие значения она принимает и какие элементы будут выбраны.
...
Кроме прямого выбора элемента, можно передать специальные последовательности, которые смогут выбрать не один, а сразу несколько элементов.
...
Похожим псевдоклассом — это `:nth-of-type(условие)`. Попробуйте в примере выше заменить `nth-child` и результат останется тем же. Но зачем нужен еще один псевдокласс с тем же функционалом?

<!-- CodePen (перезалить отдельно) -->

Приглядитесь внимательно. В случае с `nth-child` отсчет элементов начался с четного элемента. Но ведь 1 — это нечетное число. Вы будете правы с точки зрения математики, но не логики работы `nth-child`. Он выбрал все элементы с учетом того, на какой позиции они находятся относительно других элементов в блоке. По этой причине элементы 4 и 5 — нечётные, хотя и идут подряд. Логика работы следующая:

1. Первый элемент внутри блока — h2. Он стоит на нечетной позиции относительно всех элементов внутри родителя.

1. Элементы 1 и 3 — чётные, так как внутри родителя — второй и четвёртый элементом соответственно.

1. Элементы 2 и 4 — нечётные, так как внутри родителя — третий и пятый элементом.
