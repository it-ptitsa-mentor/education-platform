...
А что, если колбек-функция вернет не просто значение, а промис? Тогда параметром следующего `then()` становится [результат выполнения](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then#return_value) этого промиса. Иначе промисы были бы бессмысленны. Пример:
